const genererFigures = (fxhash) => {
    ...
    // addRoom(0, 80, 0);
    // addRoom(0, 100, 0);
    // addRoom(maxWidth, 0, 0);
    // addRoom(maxWidth, 20, 0);
    // addRoom(maxWidth, 40, 0);
    // addRoom(maxWidth, 60, 0);
    // addRoom(maxWidth, 80, 0);
    // addRoom(maxWidth, 100, 0);

    for (let i = 0; i < newRoomCount; i++) {
        for (let j = 0; j < newRoomCountX; j++) {
            let randN = fxrand();
            let randY = fxrand();
            // Randomly flip the x, z coordinate
            if (randN <= 0.5) {
                xDir = -xDir;
            }
            if (randY <= 0.5) {
            ...
            addRoom(
                i * Math.floor(fxrand() * xDir) + 1,
                j * Math.floor(fxrand() * heightOffset) + 10, -i * Math.floor(fxrand() * zDir) + 1
            );
        }
    }
